...
from decoder_planercnn.model import detection_layer, unmold_detections
...
def visualizeBatchDetection(options, config, input_dict, detection_dict, indexOffset=0, prefix='', suffix='', prediction_suffix='', write_ply=False, write_new_view=False):
    ...
    if 'mask' in input_dict:
        ...
        for box, mask in zip(boxes, masks):
            ...
            continue
        cv2.imwrite(options.test_dir + '/' + str(indexOffset) + '_segmentation_mask' + suffix + '.png', segmentation_image.astype(np.uint8)[80:560])
    ...
    if 'detection' in detection_dict and len(detection_dict['detection']) > 0:
        ...
        cv2.imwrite(options.test_dir + '/' + str(indexOffset) + '_segmentation_detection' + suffix + prediction_suffix + '.png', instance_image[80:560])
    else:
