...
from decoder_yolo.utils.datasets import *
...
def detection(save_img=False):
    ...
    # Initialize model
    model = Model(yolo_cfg=opt.cfg, midas_cfg=None, planercnn_cfg=InferenceConfig(plane_args), path=None)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        yolo_ip = img
        yolo_op, midas_op, _ = model.forward(yolo_ip, True, None)
        pred = yolo_op[0]
        ...
        midas_op = (torch.nn.functional.interpolate(
                        midas_op.unsqueeze(1),
                        size=img.shape[2:],
                        mode="bicubic",
                        align_corners=False,
                    )
        ...
        utils.write_depth(filename+'_depth', midas_op, bits=2)
